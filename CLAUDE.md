...
Stata4j is a small, dependency-free Java library for reading Stata `.dta` files. Maven project (`io.github.huapeng01016:stata4j:0.1.0`), Java 17 (`maven.compiler.release`). JUnit 5 (api/params/engine) is the only test dependency.
...
- strL cells are read as a `StrLRef(v,o)` placeholder and swapped for the `<strls>` GSO content once that section has been read. `(0,0)` means an empty string. In the data section, v comes first and then o, each in the file's byte order; the split is 4/4 for 117, 2/6 for 118 and 3/5 for 119.
...
- README.md (overview) and API.md (reference) document the public API. GETTING_STARTED.md is a tutorial. `docs/` has developer notes: `changes.md` (history and migration), `dta-format.md` (per-release layouts and assumptions) and `testing.md` (fixtures and coverage gaps). Update them when public behavior or format handling changes.
