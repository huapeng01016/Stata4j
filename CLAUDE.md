...
- **`DtaFilter`** parses `filterObservations` expressions (recursive descent; `!` > `&` > `|`) and `bind()`s them to a file's variables, giving each filtered variable a numeric or string input slot. Numeric comparisons use **Stata missing rules**: `readValue` writes a sort key for filtered numeric columns, and missing `k` maps to `missingKey(k)`, which is above every number and ordered `. < .a < ... < .z`. So the filter must never compare the `null` values returned to callers. `readData` drops failing rows as it reads. A filter on a **strL** can only be checked after `<strls>`, so `DataPass.deferred` keeps those inputs and `applyDeferredFilter` runs before `resolveStrLs`. Rows in a `DataPass` carry their file index (`getObservationIndex`).
...
- `StataReaderFilterTest` checks Stata missing semantics on `LegacyDtaBuilder` (the only source of `.a`/`.z`), and checks results against Java predicates. `DtaFilterTest` covers parsing and precedence without a file. In `@CsvSource` tables, use a delimiter other than `|` when an expression contains `|`.
